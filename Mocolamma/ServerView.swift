    ...
    var body: some View {
        VStack {
            ...
            List(serverManager.servers, selection: $listSelection) { server in
                ServerRowView(
                    server: server,
                    isSelected: server.id == serverManager.selectedServerID // API用選択状態
                )
                // 独自のハイライト実装を削除し、Listのデフォルトの選択ハイライトを使用します。
                // .listRowBackground(server.id == serverManager.selectedServerID ? Color.accentColor.opacity(0.2) : Color.clear)
                .contextMenu { // 右クリックコンテキストメニュー
                    ...
                    // 「Local」サーバーは編集も削除も不可
                    if !(server.name == "Local" && server.host == "localhost:11434") {
                        Button("Edit...") { // 編集ボタン
        ...
        .onChange(of: serverManager.selectedServerID) { oldID, newID in
            // API通信用の選択IDが変更されたら、リストのハイライトも更新
            listSelection = newID
        }

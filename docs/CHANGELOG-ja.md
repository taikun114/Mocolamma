...
#### 全般
- **iPhone Duoに対応**
  - iPhone Duoで正しく表示されるようになり、ツールバーのオーバーフローメニューの優先度が整理されたため、iPhone DuoやiPadの小さなウィンドウなどでツールバーの主要なアクションがオーバーフローメニューに入りにくくなりました。

### バグ修正と改善
#### 全般
- **サーバーアクションとモデルアクションのツールバーアイコンを改善**
  - 1.4.0で追加されたサーバーアクションとモデルアクションのツールバーアイコンが変更され、オーバーフローメニューと混同しにくくなりました。

## 1.4.0
### 新機能
#### 全般
- **iPhoneアプリのリサイズに対応**
  - iPhone Duoへの正式対応は次回以降のアップデートで対応する予定です。
- **サーバーとモデル画面のツールバーにアクションボタンを追加**
  - リスト項目を右クリックしたり長押ししたりする必要なくコンテキストメニューにアクセスできるようになりました。

#### チャット
- **テキストファイルの添付に対応**
  - チャットに対応しているすべてのモデルで`.txt`や`.md`をはじめとする、様々なテキストファイルを添付することができるようになりました。
- **PDFファイルの添付に対応**
  - チャットに対応しているすべてのモデルでPDFファイルを添付することができるようになりました。また、画像認識機能に対応したモデルでは各ページが画像としても送信されるため、テキストに加えて画像やレイアウトなども認識することができます。
- **思考レベルの選択に対応**
  - 思考対応モデルで思考レベルを「オフ」「低」「中」「高」「最高」から選択できるようになりました。また、「指定なし」は「デフォルト」に名称変更されました。

### バグ修正と改善
#### 全般
- **アプリアイコンを刷新**
  - アプリアイコンとiOSの起動画面に表示されるデザインをリニューアルしました。
- **macOSでウィンドウ位置とサイズがリセットされてしまうことがあった問題を修正**
- **ライセンス画面の見た目を改善**
- **サーバー追加編集画面のホストとモデル追加画面のモデル名入力欄でスペルが自動修正を行わないように改善**
- **APIタイムアウト時間のデフォルトを5分に変更**
  - 大容量モデルを読み込む際やプロンプト処理時間が長い時にタイムアウトエラーになる可能性が軽減しました。

#### モデル
- **環境によってモデルの変更日が正しく表示されなかった問題を修正**
- **モデルダウンロード完了後の検証中表示を改善**

#### チャットと画像生成
- **macOS Golden Gateでモデルピッカーの中身が空っぽになってしまうことがあった問題を修正**
- **macOS Tahoe以降でピッカーの中身が縮小されることがあった問題を修正**
- **macOS Golden Gateでメッセージ入力欄の見た目を改善**
- **下までスクロールボタンの見た目を改善**
- **チャット履歴や画像生成履歴を消去したときに入力されたテキストや添付されたファイルがリセットされないように改善**
- **自動追従スクロールの動作を改善**

#### チャット
- **「思考」が「指定なし」の時に思考メッセージが表示されない問題を修正**
- **ストリーミング完了時にスクロールがジャンプすることがあった問題を修正**
- **添付されているファイルアイコンの上にドロップできなかった問題を修正**
- **画像を添付するとパフォーマンスが低下してしまう問題を修正**
- **チャットを停止した後に編集・再送信すると空のメッセージバブルが残ってしまうことがあった問題を修正**

#### 画像生成
- **下までスクロールボタンが表示されたときに画像が縮小してしまう問題を修正**
